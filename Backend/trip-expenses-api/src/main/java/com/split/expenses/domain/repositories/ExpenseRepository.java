...
//Class that does data persistence, that is, sends data and fetches data from the database
public interface ExpenseRepository extends CrudRepository<Expense, Long> {
    ...
    @Query(value = "SELECT sum(e.amount) as Total, count(e.id) as QtdTotal," +
            ...
            "avg(e.amount) as media_value FROM TRIPS e WHERE status = 0", nativeQuery = true)
